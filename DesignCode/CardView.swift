        ...
        ZStack {
            TimelineView(.animation) { context in
                card.image
                    ...
                    .frame(height: isTapped ? screenSize.width - 40 : 500)
                    ...
                    .phaseAnimator([1, 2], trigger: isTapped, content: { content, phase in content.scaleEffect(phase == 2 ? 1.1 : 1)
                    })
                    .onTapGesture {
                        hasNoise.toggle()
                    }
            ...
            Circle()
                ...
                .keyframeAnimator(initialValue: AnimationValues(), trigger: isDownloading){ content, value in content.offset(x: value.position.x, y: value.position.y)
                ...
                HStack {
                    Button { hasPattern.toggle() } label: {
                        Image(systemName: "ellipsis")
                            .symbolEffect(.pulse)
                        .onTapGesture { hasPattern.toggle() }
                    }
                    .tint(.primary)
                    Divider()
